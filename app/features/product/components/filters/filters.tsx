...
function FilterBase({ searchParams }: FilterProps) {
  ...
  return (
    <div
      ...
      className="flex-1 flex-shrink-0 flex flex-col h-full"
    >
      <ScrollArea className="flex-grow">
        <div className="p-2 space-y-4">
          <div>
            <Label htmlFor="year-range">Price</Label>
            <Slider
              id="price-range"
              min={0}
              max={50000}
              step={100}
              value={[Number(optimisticFilters.yr) || 50000]}
              onValueChange={([value]) =>
            ...
            <div className="flex justify-between mt-1 text-sm text-muted-foreground">
              <span>0</span>
              <span>{optimisticFilters.yr || 50000}</span>
            </div>
          </div>
